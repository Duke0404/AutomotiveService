...
export async function getVehicle(conn, vehicleId) {
	const query = "SELECT VEHICLE_ID, VEHICLE_NAME, PRODUCTION_YEAR, TYPE_NAME, MANUFACTURER_NAME FROM VEHICLE JOIN MANUFACTURER ON MANUFACTURER_MANUFACTURER_ID = MANUFACTURER_ID JOIN VEHICLE_CLASS ON VEHICLE_CLASS_TYPE_ID = TYPE_ID WHERE VEHICLE_ID = :1"
...
export async function checkVehicleExists(conn, vehicleIdentifier, usingId = true) {
	const query = `SELECT VEHICLE_ID FROM VEHICLE WHERE ${usingId ? "VEHICLE_ID" : "VEHICLE_NAME"} = :1`
...
export async function createVehicle(conn, vehicleName, productionYear, vehicleType, manufacturerId) {
	const query = "INSERT INTO VEHICLE (VEHICLE_NAME, PRODUCTION_YEAR, VEHICLE_CLASS_TYPE_ID, MANUFACTURER_MANUFACTURER_ID) VALUES (:1, :2, :3, :4) RETURNING VEHICLE_ID INTO :5"
...
export async function updateVehicle(conn, vehicleId, vehicleName, productionYear, vehicleType, manufacturerId) {
	const query = "UPDATE VEHICLE SET VEHICLE_NAME = :1, PRODUCTION_YEAR = :2, VEHICLE_CLASS_TYPE_ID = :3, MANUFACTURER_MANUFACTURER_ID = :4 WHERE VEHICLE_ID = :5"
